...
public class dfUpgradeHelper
{
	...
	[MenuItem( "Tools/Daikon Forge/Texture Atlas/Reimport Atlas Prefabs" )]
	public static void ReimportAtlases()
	{		
		var atlases = findPrefabsOfType<dfAtlas>();
		
		EditorUtility.DisplayProgressBar( "Reimport", "Reimporting atlases", 0 );
		...
		for( int i = 0; i < count; i++ )
		{
			...
			EditorUtility.DisplayProgressBar( "Reimport", "Reimporting atlas: " + atlas.name, (float)i / (float)count );
			Debug.Log( "Reimporting Texture Atlas: " + atlas.name, atlas );
